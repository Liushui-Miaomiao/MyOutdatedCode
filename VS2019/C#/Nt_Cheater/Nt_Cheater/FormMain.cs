...
namespace Nt_Cheater {
	public partial class FormMain : Form {
		...
		private void CallBackFindProcessSuccess() {
			MyUtils.GetPidByProcessName(
				PROCESS_NAME, 
				CallBackGetProcessPidSuccess, 
				CallBackFail
			);
		}
		/// <summary>
		...
		private void CallBackGetProcessPidSuccess(int pid) {
			...
			MyUtils.GetProcessBaseAddr(
				PROCESS_NAME, 
				CallBackGetProcessBaseAddrSuccess, 
				CallBackFail
			);
		}
		/// <summary>
		...
		private void CallBackGetProcessBaseAddrSuccess(IntPtr baseAddr) {
			...
			MyToolStripStatusLabel.Text = 
				"进程Pid：" + _myGame.pid + "，" +
				"当前难度：" + _myGame.Difficulty
			;
		...
		private void CheckboxSmooth_CheckedChanged(object sender, EventArgs e) {
			...
			_myGame.Quality = 
				self.Checked ? 
					MyGame.QUALITY_SMOOTH : 
					MyGame.QUALITY_NO_SMOOTH
			;
		}
		/// <summary>
		///	定时器_重新寻找游戏
